"""Script to optimize a DraftKings lineup with an Oakland (ATH) 5-man stack and a Pittsburgh (PIT) 3-man stack."""
import json
import csv
...
def main():
    ...
    prob = pulp.LpProblem("DK_DFS_OAK_PIT", pulp.LpMaximize)
    x = {}
    ...
    # Max 5 players per team (applies to both SP and Hitters)
    ...
    # Pittsburgh (PIT) 3-man Hitter Stack Constraint
    prob += pulp.lpSum(x[(p_name, pos)] for p_name, p_info in players.items() if p_info["team"] == "PIT" and not p_info["is_pitcher"] for pos in p_info["positions"] if pos != "SP") == 3

    # Force Paul Skenes and Emerson Hancock to be selected
    ...
    if status == pulp.LpStatusOptimal:
        ...
        print("\n--- OAKLAND 5-MAN + PITTSBURGH 3-MAN STACK LINEUP ---")
