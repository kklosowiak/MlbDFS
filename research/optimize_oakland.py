"""Script to optimize a DraftKings lineup with an Oakland (ATH) 5-man stack."""
import json
import csv
...
def main():
    ...
    for p in matched_pitchers:
        # Avoid trap pitchers like Sugano
        if p.get("is_trap"):
            continue
        players[p["dk_name"]] = {
            "name": p["dk_name"],
    ...
    prob = pulp.LpProblem("DK_DFS_OAK", pulp.LpMaximize)
    x = {}
