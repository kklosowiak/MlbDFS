...
def solve_lineup(pitchers, hitters, stack1_team=None, stack1_size=0, stack2_team=None, stack2_size=0):
    ...
    # Restrict to top 4 SP pairs, top 4 hitters per position, top 6 OFs for ultra-fast run
...
# Run optimizations
results = {}
results["Yankees (5) + Blue Jays (3) Stack"] = solve_lineup(matched_pitchers, matched_hitters, "NYY", 5, "TOR", 3)
results["Pirates (5) + Blue Jays (3) Stack"] = solve_lineup(matched_pitchers, matched_hitters, "PIT", 5, "TOR", 3)
...
for name, res in results.items():
    ...
    if res:
        ...
        for pos, p in display_list:
            print(f"- {pos}: {p['name']} ({p['team']}) - ${p['salary']} (Score: {p['score']:.1f})")
    else:
